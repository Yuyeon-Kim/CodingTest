...
int binarySearch(int arr[], int firstIndex, int lastIndex, int target)
{
  ...
  {
    int mindex = (firstIndex + lastIndex) / 2; // 수정
    printf("binarySearch Start first : %d, last : %d, target : %d\n", firstIndex, lastIndex, target);
    ...
    {
      printf("target smaller, list[%d] : %d, target : %d\n", mindex, arr[mindex], target);
      binarySearch(arr, firstIndex, mindex - 1, target);
    }
    else
    {
      printf("target larger, list[%d] : %d, target : %d\n", mindex, arr[mindex], target);
...
int main()
{
  ...
  scanf("%d", &target);
  int a = binarySearch(list, 0, n - 1, target);
  printf("%d\n", a);
  if (a == 1) // 수정
  {
