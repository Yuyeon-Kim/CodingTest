        ...
        for n in range(5):           # 조건들에 대해 조합을 이용해서  
            ...
            for c in combi:
                ...
                for v in c:          # '-'를 포함한 새로운 조건을 만들어냄.
                    ...
                    # - backend junior pizza, java - junior pizza ... - - - -
                    # 의 쿼리가 나오면 해당 조건이 만족됨.
                changed_t_c = '/'.join(t_c)
                if changed_t_c in db:     # 모든 조건의 경우에 수에 대해 딕셔너리
                    db[changed_t_c].append(score) # 조건이 이미 있으면, 점수 리스트에 더함
                else:
                    db[changed_t_c] = [score] # 조건들을 키값으로, 점수를 값으로 저장
    ...
    for q in query:                       # query의 모든 조건에 대해서
        ...
        if qry_cnd in db:                 # 딕셔너리 내에 값이 존재한다면,
            ...
            if len(data) > 0:          
                ...
                while start != end and start != len(data): # 이진 탐색 알고리즘
...
#출처 : https://dev-note-97.tistory.com/131